...
public class LetvSimplePlayBoard {
    ...
    private Callback surfaceCallback = new Callback() {
        ...
        public void surfaceDestroyed(SurfaceHolder holder) {
        	Log.d("DetailBoard","sufaceDestory----1");
        	surfaceHolder=null;
        	Log.d("DetailBoard","sufaceDestory----2");
            stopAndRelease();
            Log.d("DetailBoard","sufaceDestory----3");
        }
        ...
        public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        	Log.d("DetailBoard","surfaceChanged----W:"+width+" H:"+Constants.ScreenHeight);
        	if((width == Constants.ScreenHeight) || (width+Constants.naviBarHeight == Constants.ScreenHeight))
        	{
        		isFullScreen=true;
        	}
    ...
    public void resetToActive(Context context,Bundle mBundle)
    {
    	player.stop();
        player.reset();
        player.release();
    ...
    private void pause() {
        ...
        Log.d("DetailBoard","pause --- 2");
        if (skin != null) {
            skin.onPause();
        }
        Log.d("DetailBoard","pause --- 3");
        if (player != null) {
            player.pause();
        }
        Log.d("DetailBoard","pause --- 4");
    }

    private void destroy() {
    	Log.d("DetailBoard","destory --- 1");
        if (skin != null) {
            skin.onDestroy();
        }
        Log.d("DetailBoard","destory --- 2");
        if (playContext != null) {
            playContext.destory();
        }
        Log.d("DetailBoard","destory --- 3");
    }
    ...
    public void stopAndRelease() {
        if (player != null) {
        	Log.d("DetailBoard","stopAndRelease----1");
            lastPosition = player.getCurrentPosition();
            Log.d("DetailBoard","stopAndRelease----2");
            player.stop();
            Log.d("DetailBoard","stopAndRelease----3");
            player.reset();
            Log.d("DetailBoard","stopAndRelease----4");
            player.release();
            Log.d("DetailBoard","stopAndRelease----5");
            player = null;
        }
    }
