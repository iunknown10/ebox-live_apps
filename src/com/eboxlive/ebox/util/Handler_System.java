...
public class Handler_System {
	...
	private static final String tag = "Handler_System";
	...
	public static String getAppName(String packageName) {
		...
		if (packageName == null) {
			packageName = OcApplication.getApplication().getPackageName();
		}

		try {
			PackageManager packageManager = OcApplication.getApplication().getPackageManager();
			PackageInfo packageInfo = packageManager.getPackageInfo(packageName, 0);
			applicationName = OcApplication.getApplication().getString(packageInfo.applicationInfo.labelRes);
		} catch (Exception e) {
	...
	public static boolean isRelease(String signatureString) {
		...
		try {
			PackageManager pm = OcApplication.getApplication().getPackageManager();
			PackageInfo pi = pm.getPackageInfo(OcApplication.getApplication().getPackageName(), PackageManager.GET_SIGNATURES);
	...
	public static HashMap<String, Integer> getDisplayMetrics() {
		if (map == null) {
			...
			Display display = ((WindowManager) OcApplication.getApplication().getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
	...
	public static float getWidthRoate() {
		...
		return (map.get(systemWidth) * 1.00f) / Constants.ScreenWidth;
	}
	...
	public static float getRoate() {
		...
		float w = (map.get(systemWidth) * 1.00f) / Constants.ScreenWidth;
		float h = (map.get(systemHeight) * 1.00f) / Constants.ScreenHeight;
		return w>h?w:h;
	}
	...
	public static float getHeightRoate() {
		...
		return (map.get(systemHeight) * 1.00f) /  Constants.ScreenHeight;
	}
	...
	public static int dip2px(float dipValue) {
		final float scale = OcApplication.getApplication().getResources().getDisplayMetrics().density;
	...
	public static int getBarHeight() {
		...
		try {
			...
			sbar = OcApplication.getApplication().getResources().getDimensionPixelSize(x);
		} catch (Exception e1) {
	...
	public static void Vibrate(long milliseconds) {
		Vibrator vib = (Vibrator) OcApplication.getApplication().getSystemService(Service.VIBRATOR_SERVICE);
	...
	public static void init() {
		mTm = (TelephonyManager) OcApplication.getApplication().getSystemService(Context.TELEPHONY_SERVICE);
		mIMEI = mTm.getDeviceId();
		mMobileVersion = mTm.getDeviceSoftwareVersion();
		//mCellinfos = mTm.getNeighboringCellInfo(); //需要高精度定位权限
		mNetwrokIso = mTm.getNetworkCountryIso();
		mSIM = mTm.getSimSerialNumber();
		mDeviceID = getDeviceId();
		try {
			ConnectivityManager cm = (ConnectivityManager) OcApplication.getApplication().getSystemService(Context.CONNECTIVITY_SERVICE);
	...
	private static String getDeviceId() {
		return Secure.getString(OcApplication.getApplication().getContentResolver(), Secure.ANDROID_ID);
	}
	...
	public static String getPhoneSettings() {
		...
		String str = Secure.getString(OcApplication.getApplication().getContentResolver(), Secure.BLUETOOTH_ON);
		buf.append("蓝牙:");
		if (str.equals("0")) {
			buf.append("禁用");
		} else {
		...
		//
		str = Secure.getString(OcApplication.getApplication().getContentResolver(), Secure.BLUETOOTH_ON);
		buf.append("WIFI:");
		buf.append(str);

		str = Secure.getString(OcApplication.getApplication().getContentResolver(), Secure.INSTALL_NON_MARKET_APPS);
		buf.append("APP位置来源:");
		buf.append(str);
